...
const socket = io("http://13.201.0.34:3001", {
...
function getGroups() {
  axios
    .get("http://13.201.0.34:3001/group/get-group", {
      headers: {
        Authorization: token,
      },
    })
...
document.getElementById("addUserBtn").addEventListener("click", (event) => {
  ...
  axios
    .post(`http://13.201.0.34:3001/admin/add-member/${groupId}`, { email })
    .then((response) => {
...
document.getElementById("removeUserBtn").addEventListener("click", (event) => {
  ...
  axios
    .post(`http://13.201.0.34:3001/admin/remove-member/${groupId}`, { email })
    .then((response) => {
      alert(response.data.message);
    })
    .catch((err) => {
...
document.getElementById("promoteUserBtn").addEventListener("click", (event) => {
  ...
  axios
    .put(`http://13.201.0.34:3001/admin/promote-member/${groupId}`, { email })
    .then((response) => {
      alert(response.data.message);
    })
    .catch((err) => {
...
function isMemberAdmin() {
  ...
  axios
    .get(`http://13.201.0.34:3001/admin/is-admin/${groupId}`, {
      headers: {
        Authorization: token,
      },
    })
...
document.getElementById("fileSendBtn").addEventListener("click", () => {
  ...
  const urlParams = new URLSearchParams(window.location.search);
  const groupId = Number(urlParams.get("groupId"));
  axios.post(`http://13.201.0.34:3001/file/upload/${groupId}`, fileData, {
    headers: {
      Authorization: token,
